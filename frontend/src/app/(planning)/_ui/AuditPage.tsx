...
import type { AuditEvent, AuditEventType, JobSummary } from '@/types/pipeline';
import { cn, formatKST } from '@/lib/utils';
import {
  ArrowUp, ArrowDown, ArrowUpDown, FileText, X, Calendar,
  PlusCircle, CheckCircle, PlayCircle, XCircle, RefreshCw, Bell, UserCheck,
  ExternalLink,
} from 'lucide-react';
...
const EVENT_CONFIG: Record<AuditEventType, { label: string; color: string; bgColor: string; icon: React.ElementType; description: string }> = {
  JOB_CREATED: { label: 'Job 생성', color: 'text-muted-foreground', bgColor: 'bg-muted/50 text-muted-foreground', icon: PlusCircle, description: '새로운 처리 작업이 생성되었습니다. 파이프라인 자동 트리거 또는 수동 실행에 의해 발생합니다.' },
  JOB_ASSIGNED: { label: 'Job 할당', color: 'text-accent', bgColor: 'bg-accent/15 text-accent', icon: UserCheck, description: '처리 작업이 워커에 할당되어 실행 대기 중입니다.' },
  JOB_COMPLETED: { label: 'Job 완료', color: 'text-success', bgColor: 'bg-success/15 text-success', icon: CheckCircle, description: '처리 작업이 모든 단계를 성공적으로 완료했습니다.' },
  JOB_FAILED: { label: 'Job 실패', color: 'text-destructive', bgColor: 'bg-destructive/15 text-destructive', icon: XCircle, description: '처리 작업이 오류로 인해 실패했습니다. 재시도 횟수 초과 시 Dead Letter로 이동됩니다.' },
  PIPELINE_STARTED: { label: '파이프라인 시작', color: 'text-accent', bgColor: 'bg-accent/15 text-accent', icon: PlayCircle, description: '파이프라인이 시작되어 데이터 처리 워크플로우가 실행됩니다.' },
  PIPELINE_REPROCESSED: { label: '재처리', color: 'text-muted-foreground', bgColor: 'bg-muted/50 text-muted-foreground', icon: RefreshCw, description: '운영자에 의해 재처리가 요청되었습니다. 지정된 레벨부터 처리가 다시 시작됩니다.' },
  ALERT_DISPATCHED: { label: '알림 발행', color: 'text-destructive', bgColor: 'bg-destructive/15 text-destructive', icon: Bell, description: '시스템 이상 상황이 감지되어 운영자에게 알림이 발행되었습니다.' },
};
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-5">
      ...
      <div className="space-y-2">
        <span className={cn('inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium', config.bgColor)}>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <DetailField
          label="Job ID"
          value={
            <a href={`${base}/console?jobId=${event.jobId}`} className="inline-flex items-center gap-1 font-mono text-accent hover:underline text-xs">
              {event.jobId}
      ...
      <div className="pt-1 border-t border-border">
        <div className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-2.5">
          Job 이벤트 타임라인
          {!relatedLoading && <span className="font-mono font-normal normal-case ml-1">({relatedEvents.length}건)</span>}
        </div>
              ...
              {sortedRelatedEvents.map((evt) => {
                ...
                return (
                  ...
                  >
                    {/* Timeline dot */}
                    <div className={cn(
                      'absolute left-0 top-[11px] w-[11px] h-[11px] rounded-full border-2 shrink-0',
                      isCurrent
      ...
      <div className="pt-1 border-t border-border">
        ...
        <div className="space-y-1">
          <a
            href={`${base}/console?jobId=${event.jobId}`}
...
export default function AuditPage() {
  ...
  useEffect(() => {
    (async () => {
      const [allRes, jobsRes] = await Promise.all([
        service.감사로그를_조회한다({ size: 500 }),
  ...
  }, [service, filterJobId, filterEventType, filterFrom, filterTo, page, sortBy, sortOrder]);
  ...
  }, [service, selectedEvent?.jobId, selectedEvent?.id]);
          ...
          {totalPages > 1 && (
            <div className="flex items-center gap-1.5 ml-auto">
              <button
                ...
                className="px-2 py-1 text-xs rounded-md border border-border text-muted-foreground hover:bg-muted/30 disabled:opacity-30 disabled:cursor-not-allowed"
              >
                이전
              </button>
        ...
        <div className="flex-1 flex overflow-hidden">
                ...
                <tbody>
                  {events.map((evt) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-2.5 text-muted-foreground whitespace-nowrap">{formatKST(evt.timestamp)}</td>
                        <td className="px-4 py-2.5">
                          <span className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium', config.bgColor)}>
                            <config.icon className="w-3 h-3" />
                            {config.label}
                          </span>
                        </td>
                        <td className="px-4 py-2.5">
                          <a
                            href={`${base}/console?jobId=${evt.jobId}`}
          ...
          <div
            ...
            className={cn(
              ...
              panelOpen
                ? 'w-[380px] min-w-[380px] opacity-100 translate-x-0'
                : 'w-0 min-w-0 opacity-0 translate-x-4 border-l-0',
            )}
          >
            {selectedEvent && (
              <>
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
                  <div className="flex items-center gap-2 min-w-0">
                    {(() => { const C = EVENT_CONFIG[selectedEvent.eventType].icon; return <C className="w-4 h-4 text-accent shrink-0" />; })()}
                    <span className="text-xs font-semibold text-foreground truncate">이벤트 상세</span>
                  </div>
                  <button onClick={() => setSelectedEvent(null)} className="p-1 rounded-md hover:bg-muted/50 transition-colors">
