...
import type { Product, ProductLevel, JobSummary } from '@/types/pipeline';
import { PRODUCT_LEVEL_LABELS } from '@/types/pipeline';
import { cn, formatKST, formatDuration } from '@/lib/utils';
import {
  Package, Search, Download, RefreshCw, X, CheckCircle, XCircle,
  Image, MapPin, Clock, Ruler, Eye,
} from 'lucide-react';
...
function ProductStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={cn('inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium', styles[status] ?? 'bg-muted text-muted-foreground')}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-sm mx-4 p-5">
          ...
          <label className="block">
            ...
            >
              {availableLevels.map((l) => (
                <option key={l} value={l}>{PRODUCT_LEVEL_LABELS[l]} ({l})</option>
              ))}
            </select>
          </label>
        ...
        <div className="flex justify-end gap-2 mt-4">
          <button onClick={onCancel} className="px-3 py-1.5 rounded-md text-xs text-muted-foreground hover:bg-muted/30 transition-colors">
            취소
          </button>
          <button
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <MetaItem label="Job ID" value={product.jobId} href={`${base}/console?jobId=${product.jobId}`} />
          </div>
        </div>
        ...
        <div className="space-y-2">
          <h4 className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1">
            <MapPin className="w-3 h-3" />공간 범위
          </h4>
          <div className="text-[10px] font-mono text-foreground bg-background rounded-md px-3 py-2 border border-border">
            W: {product.spatialExtent.west.toFixed(4)}° &nbsp;
            S: {product.spatialExtent.south.toFixed(4)}°<br />
            E: {product.spatialExtent.east.toFixed(4)}° &nbsp;
            N: {product.spatialExtent.north.toFixed(4)}°
          </div>
        </div>
        ...
        {product.quality && (
          <div className="space-y-2">
            <h4 className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1">
              <Eye className="w-3 h-3" />품질 검증 (REQ-FUNC-023)
            </h4>
              ...
              <table className="w-full text-[10px]">
                ...
                <tbody>
                  <tr className="border-t border-border/50">
                    ...
                    <td className="px-3 py-1.5 text-right font-mono text-foreground">{product.quality.nesz.value.toFixed(1)} {product.quality.nesz.unit}</td>
                  ...
                  <tr className="border-t border-border/50">
                    ...
                    <td className="px-3 py-1.5 text-right font-mono text-foreground">{product.quality.pslr.value.toFixed(1)} {product.quality.pslr.unit}</td>
                  ...
                  <tr className="border-t border-border/50">
                    ...
                    <td className="px-3 py-1.5 text-right font-mono text-foreground">{product.quality.geometricAccuracy.value.toFixed(1)} {product.quality.geometricAccuracy.unit}</td>
...
function MetaItem({ label, value, href }: { label: string; value: string; href?: string }) {
  return (
    <div>
      ...
      {href ? (
        <a href={href} className="text-xs font-medium text-accent hover:underline">{value}</a>
      ) : (
...
export default function ProductsPage() {
  ...
  async function handleDownload(product: Product) {
    ...
    if (res.success && res.data) {
      setToast({ message: `다운로드 링크 생성 완료 (${Math.floor(res.data.expiresIn / 60)}분 후 만료)`, type: 'success' });
    ...
    <div className="h-full flex">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={cn('flex-1 flex flex-col overflow-hidden', panelMounted && 'max-w-[calc(100%-600px)]')}>
          ...
          <div className="flex items-center gap-3 px-5 py-2.5 border-b border-border shrink-0 flex-wrap">
            ...
            >
              ...
              {LEVELS.map((l) => <option key={l} value={l}>{PRODUCT_LEVEL_LABELS[l]}</option>)}
            </select>
            <select
              ...
              <tbody>
                {filtered.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-center"><ProductStatusBadge status={p.status} /></td>
                    ...
                    <td className="px-5 py-2.5">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); setReprocessTarget(p); }}
        ...
        {panelMounted && selectedProduct && (
          <div
            className={cn(
              'w-[600px] shrink-0 transition-all duration-200 ease-out overflow-hidden',
