...
- [x] 캔버스 그래프 맨 왼쪽에 "원시 데이터 수신" 트리거 노드가 표시됨
- [x] 트리거 노드 클릭 시 우측 패널에 `receivedAt`, `rawDataPath` 표시
- [x] 트리거 노드에는 삭제 버튼이 없음
- [x] Job 실행 뷰에서 트리거 노드 상태가 `COMPLETED`로 표시됨
...
- [x] "재처리" 버튼 → 전체/부분 선택 드롭다운으로 변경
- [x] 부분 재처리 클릭 시 target_level 선택 다이얼로그 열림
- [x] LEVEL_0 선택 시 추가 경고 문구 표시
- [x] `부분_재처리를_요청한다()` mock 구현 존재
...
- [x] CSC-07 노드가 회색 스타일로 렌더링됨
- [x] 노드 호버 시 "SI-08 스키마 미확정 — 상태 표시 불가" 툴팁 표시
- [x] 다른 CSC 노드의 스타일에 영향 없음
...
- [x] `types/pipeline.ts`의 `TargetCsc` 정의에 위 주석 추가
