...
- 명령 (Bash 툴 / git-bash 호환). **빌드 컨텍스트는 repo root** (CSC natives COPY 위함). H5 입력은 컨테이너로 직접 업로드(multipart)하므로 호스트 볼륨 마운트는 불필요.
  ```
  docker rm -f sdpe-frontend 2>/dev/null; docker build -t sdpe-frontend:latest -f frontend/Dockerfile . && docker run -d --name sdpe-frontend --restart=unless-stopped -p 3010:3000 sdpe-frontend:latest
  ```
- 컨테이너는 `http://localhost:3010` 에 노출된다.
- CSC-03/04 시연 통합은 `frontend/docs/csc-demo-integration.md` 참고.
