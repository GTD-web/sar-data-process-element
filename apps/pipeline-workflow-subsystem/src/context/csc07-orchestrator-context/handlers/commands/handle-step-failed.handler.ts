...
import { PIPELINE_EXECUTION_REPOSITORY, type IPipelineExecutionRepository } from '@sdpe/pipeline';
import { RETRY_EVALUATOR, type IRetryEvaluator } from '@sdpe/retry-policy';
import { ALERT_CONDITION_EVALUATOR, type IAlertConditionEvaluator, ALERT_DISPATCHER, type IAlertDispatcher } from '@sdpe/alert';
...
export class HandleStepFailedHandler implements ICommandHandler<HandleStepFailedCommand> {
  private readonly logger = new Logger(HandleStepFailedHandler.name);
