...
| 보고서 | GitLab Pages (`http://192.168.10.11:8888/ground-segment/sar-data-process-element/`) |
...
| Remote 이름 | URL | 용도 |
|-------------|-----|------|
| `origin` | `http://192.168.10.11/ground-segment/sar-data-process-element.git` | 메인 (GitLab) |
...
현재: `http://192.168.10.11:8888/ground-segment/sar-data-process-element/`

> **그룹 경로 주의**: GitLab 웹 UI에서 그룹 이름을 "SDPE"로 변경했지만,
> 내부 경로(path)는 `ground-segment`로 남아 있음. GitLab에서 이름(name)과 경로(path)는 별개.
> 경로 변경: **그룹 Settings → General → Advanced → Change group URL**
> 단, 변경 시 git remote URL, CI 설정, Pages URL 모두 변경 필요.
...
> Project.find_by_full_path('ground-segment/sar-data-process-element').pages_deployed?
> puts Project.find_by_full_path('ground-segment/sar-data-process-element').pages_url
```
...
- [ ] `ground-segment` 그룹 경로를 `SDPE`로 변경 검토 (git remote URL 변경 필요)
